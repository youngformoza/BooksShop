...
COMMENT ON TABLE author IS
    'entity for books authors, contains: authors full name, popularity (how many people read authors books) and country, PK - author_id';
...
COMMENT ON TABLE book IS
    'entity for type of products - books, contains: name, ISBN, date of publication, FK: book_form_id, paper_type_id, book_cover_type_id, genre_id and primary foreign key: product_id, indexes for all foreign keys';
...
COMMENT ON TABLE book_cover_type IS
    'entity for cover types of books, contains name and PK - cover_book_id';
...
COMMENT ON TABLE book_form IS
    'entity for books forms, contains name and PK - form_id';
...
ALTER TABLE customer ADD("NUMBER" LONG NOT NULL);
...
COMMENT ON TABLE customer IS
    'entity for customers, contains: name, gender, age, profession and phone number and PK: customer_id';
...
COMMENT ON COLUMN customer."NUMBER" IS
    'phone number in form 9876543210';
...
ALTER TABLE employee RENAME COLUMN experince TO experience;

ALTER TABLE employee ADD CHECK ( employee_id BETWEEN 1 AND 2147483648 );

ALTER TABLE employee ADD CHECK ( age BETWEEN 14 AND 99 );

ALTER TABLE employee ADD CHECK ( experience BETWEEN 0 AND 60 );

ALTER TABLE employee ADD CHECK ( position_id BETWEEN 1 AND 2147483648 );

COMMENT ON TABLE employee IS
    'entity for employees, contains: age, experience, full name, gender, FK: position_id, PK: employee_id, indexes for all foreign keys';
...
COMMENT ON COLUMN employee.experience IS
    'work experience';
...
ALTER TABLE genre ADD CHECK ( genre_id BETWEEN 1 AND 2147483648 );

COMMENT ON TABLE genre IS
    'entity for books genres, contains name and PK - genre_id';
...
COMMENT ON TABLE gift_card IS
    'entity for type of products - gift cards, contains: FK: type_of_cover_id and primary foreign key: product_id, indexes for all foreign keys';
...
COMMENT ON TABLE kmtm_author_book IS
    'auxiliary entity many-to-many author_book, contains 2 primary foreign keys from entites author and book';
...
COMMENT ON TABLE kmtm_product_order IS
    'auxiliary entity many-to-many product_order, contains: quantity of product in order, 2 primary foreign keys from entites order and product';
...
COMMENT ON TABLE "order" IS
    'entity for orders, contains: date and time, discount, FK: employee_id and customer_id, PK: order_id, indexes for all foreign keys';
...
COMMENT ON TABLE paper_type IS
    'entity for books paper types, contains: name and PK - paper_type_id';
...
COMMENT ON TABLE position IS
    'entity for employees positions, contains: duties, name of position and PK: position_id';
...
ALTER TABLE product MODIFY price Number(7,2);
...
COMMENT ON TABLE product IS
    'entity for products, contains: price, producer and PK: product_id';
...
COMMENT ON TABLE stationery IS
    'entity for type of products - stationery, contains: name, FK:stationery_type_id (fk_stationery_stat_type) and primary foreign key: product_id, indexes for all foreign keys';
...
ALTER TABLE stationery_type ADD CHECK ( type_stat_id BETWEEN 1 AND 2147483648 );

COMMENT ON TABLE stationery_type IS
    'entity for stationery types, contains: name and PK - type_stat_id';
...
COMMENT ON TABLE type_of_cover IS
    'entity for gift cards, contains: name and PK - cover_card_id';
